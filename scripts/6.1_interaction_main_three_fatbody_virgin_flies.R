...
# ----------------------------------------------------------------------------
  try((ancova_model <- aov(gene ~ NV*clusters +NV + clusters+identity, data = data)),silent = T)
...
# ----------------------------------------------------------------------------
  ...
  assoc_results[length(assoc_results$Pvalue)+1, ] = c(d$`Pr(>F)`[5])
  print(i)
}
...
conditions <- as.data.frame(df_metadata$virus_NV)
...
fatbody_v$NV <- df_metadata$virus_NV
...
data_plot3$Process <- factor(
  data_plot3$Process,
  levels = data_plot3$Process[order(data_plot3$Score)]
)
...
# ----------------------------------------------------------------------------
ggplot(data_plot3, aes(x = Score, y = Process, fill = Score)) +
